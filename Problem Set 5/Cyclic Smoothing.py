    ...
    while offset >= tolerance:
        ...
        for i in range(dim_Row):
            for j in range(dim_Col):
                ...
                tempPath = newpath[i][j]
                newpath[i][j] += weight_data * (path[i][j] - newpath[i][j]) + weight_smooth * (newpath[index_prev][j] + newpath[index_next][j] - 2.0 * newpath[i][j])

                offset += abs(tempPath - newpath[i][j])
    #add new path track to array
    for i in range(dim_Row):
        x_trajectory.append(newpath[i][0])
        y_trajectory.append(newpath[i][1])
    x_trajectory.append(newpath[0][0])
    y_trajectory.append(newpath[0][1])

    return x_trajectory,y_trajectory,newpath  # Leave this line for the grader!
...
def evaluate_Smooth (test_path, answer_path):

    x_trajectory,y_trajectory,smooth_result = smooth(test_path)
    print_paths(test_path,smooth_result, answer_path)
    solution_check(smooth_result, answer_path)
...
evaluate_Smooth(testpath2, answer2)
