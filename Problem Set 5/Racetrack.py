...
class robot:
    ...
    def cte(self, radius):
        ...
        if (self.x < radius):
            # left semi circle
            # center of circle is r,r.  Compute robot distance
            # from this point.  Distance to closest point on circle
            # will then be d - r
            # If we are outside the track, we want a positive err val
            # to steer correctly.

            d = sqrt((self.x - radius) ** 2 + (self.y - radius) ** 2)
            CTE = d - radius

        elif (self.x > 3 * radius):
            # right semi circle -- centered at [3r,r]
            d = sqrt((self.x - 3 * radius) ** 2 + (self.y - radius) ** 2)
            CTE = d - radius

        else:
            # must be on a straightaway
            if ((self.orientation > pi / 2.0) and (self.orientation <= 3 * pi / 2.0)):
                # orientation is left so assume lower half of the track
                # print "straight - lower"
                CTE = 0.0 - self.y
            else:
                # straightaway on the upper half
                # print "straight - upper"
                CTE = self.y - 2 * radius
        return CTE
...
def run(params, radius, printflag=False):
    ...
    for i in range(N * 2):
        ...
        if printflag:
            print ('iter = %d, Point %s  CTE = %.5f steer=%.5f' % (i, myrobot, crosstrack_error, steer))
...
# plt.plot(x_trajectory, CTE, 'r', label='CTE')
plt.legend()
plt.show()
